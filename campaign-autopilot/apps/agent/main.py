...
guardrails = {"max_daily_spend": 500.0, "min_confidence": 0.7, "autonomy_level": "assisted"}
...
def approve(issue_id: str) -> dict[str, object]:
    ...
    event = {"actor": "human", "event": "action.approved", "issue_id": issue_id, "confidence": 0.92, "rationale": "Human approved a simulated creative refresh; no external platform was changed."}
    audit_log.append(event)
    return {"issue_id": issue_id, "status": "approved", "simulated": True, "audit": event, "message": "Approval recorded in the demo audit trail; no ad platform was changed."}


@app.get("/api/settings/guardrails")
def get_guardrails() -> dict[str, object]:
    return {**guardrails, "mode": "seed"}


@app.put("/api/settings/guardrails")
def update_guardrails(update: GuardrailUpdate) -> dict[str, object]:
    guardrails["max_daily_spend"] = update.max_daily_spend
    audit_log.append({"actor": "human", "event": "guardrails.updated", "max_daily_spend": update.max_daily_spend, "rationale": "Customer updated the daily spending limit."})
    return {**guardrails, "mode": "seed"}
